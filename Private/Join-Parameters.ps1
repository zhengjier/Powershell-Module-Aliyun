...
function Join-Parameters(){
    ...
    $parameters.PSObject.Properties | ForEach-Object{
        ...
        if ($_.Name -eq 'PolicyDocument') {
            $object | Add-Member -MemberType NoteProperty -Name 'Key' -Value $_.Name
            $object | Add-Member -MemberType NoteProperty -Name 'Value' -Value $(Update-String $([System.Uri]::EscapeDataString($_.Value)) -PolicyDocument)
